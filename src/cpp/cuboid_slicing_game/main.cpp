// #cpp | #kattis | #game_theory
// Cuboid Slicing Game |
// https://open.kattis.com/contests/s82cve/problems/cuboidslicinggame/statistics
...
void compute() {
    g[0][0][0] = 0;
    FOR(i, 1, MAXDIM)
    FOR(j, 1, MAXDIM) g[1][i][j] = g[i][1][j] = g[i][j][1] = 300;
    FOR(x, 1, MAXDIM)
    FOR(y, 1, MAXDIM) FOR(z, 1, MAXDIM) g[x][y][z] = getGrundyValue(x, y, z);
}
