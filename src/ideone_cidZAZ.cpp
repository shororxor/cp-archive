...
struct Node {
    ...
    static Node merge(Node& L, Node& R, int l, int r) {
        Node ans(0, 0, 0, 0, 0);
        ans.val = L.val + R.val;
        ans.first = L.first;
        ans.last = R.last;
        ...
        if (L.last <= R.first) {
            ...
            int m = (l + r) >> 1;
            if (L.pref == (m - l + 1)) ans.pref += R.pref;
            if (R.suff == (r - (m + 1) + 1)) ans.suff += L.suff;
        }
        return ans;
    }
};
...
struct SegTree {
    ...
    void update(int i, int l, int r, int u, int v, int c) {
        ...
        nodes[i] = Node::merge(nodes[i << 1], nodes[i << 1 | 1], l, r);
    }
